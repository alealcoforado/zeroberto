...
class ZeroBERTo(nn.Module):
    def __init__(self, classes_list, embeddingModel=None, contrastiveModel=None, hypothesis_template="{}.",
        ...
        if embeddingModel == None:
            ...
            self.embeddingModel = SentenceTransformer(embeddingModel, device=device)
        ...
        if contrastiveModel == None:
            ...
            self.contrastiveModel = SetFitModel.from_pretrained(contrastiveModel)
        # use_differentiable_head=True,
        # head_params={"out_features":len(classes_list)})
    ...
    def fit(self, sentences, batch_size=8, epochs=10):
        ...
        train_loss = losses.DenoisingAutoEncoderLoss(self.embeddingModel,
                                                     decoder_name_or_path='sentence-transformers/stsb-xlm-r-multilingual',
                                                     tie_encoder_decoder=True)  # decoder_name_or_path=model_name
    ...
    def create_queries(self, classes_list, hypothesis):
        ...
        # print(queries)
        emb_queries = self.embeddingModel.encode(sentences=queries, convert_to_tensor=True, normalize_embeddings=True,
                                                 device=device)
        # print(emb_queries)
        print("queries")
        return emb_queries
        ...
        if (self.average_sentence_embeddings == True):
            if '.' in x:
                ...
                for sentence in sentences:
                    encoded_sentence = self.embeddingModel.encode(sentence, convert_to_tensor=True,
    ...
    def get_top_n_results(self, dataframe_results, ascending=False, top_n=1):
        df_top_n = dataframe_results.sort_values(['probability', 'prediction_code'], ascending=ascending).groupby(
            'prediction_code').head(top_n)
        accuracy_top_n = evaluation_metrics.get_metric(df_top_n['prediction_code'].to_list(),
                                                       df_top_n['class_code'].to_list())
    ...
    def runLabeling(self):
        ...
        for text in self.labeling_dataset[self.config['data_col']].to_list():
            ...
            if len(preds) % 500 == 0:
                ...
                print("Preds:", len(preds), " - Total time:", round(t1, 2), "seconds" + " - ETA:", round(eta, 1),
                      "minutes")
    ...
    def evaluateLabeling(self, ascending=False, top_n=16):
        ...
        final_result_df_encoded = evaluation_metrics.Encoder(final_result_df, self.labelEncoder,
                                                             columnsToEncode=['prediction'])
    ...
    def getPredictions(self):
        ...
        if setfit_trainer.column_mapping is not None:
            eval_dataset = setfit_trainer._apply_column_mapping(setfit_trainer.eval_dataset,
                                                                setfit_trainer.column_mapping)
